...
export async function generateMetadata({
  params,
}: ArchivoAlbumPageProps): Promise<Metadata> {
  const { id } = await params;
  const post = await getArchivoAlbumMeta(id);

  if (!post || post.kind !== ARCHIVO_ALBUM_KIND) {
    return {
  ...
  const title = post.title ? `${post.title} | archivo` : "album | archivo";
  const description =
    getExcerpt(post.description) || formatArchivoTimestamp(post.takenAt);
  ...
  return {
    ...
    openGraph: {
      ...
      title,
      type: "article",
      url: `https://haaaaaaammmm.com${ARCHIVO_PATH}/album/${post.id}`,
    },
    title,
    twitter: {
...
export default async function ArchivoAlbumPage({
  params,
  searchParams,
}: ArchivoAlbumPageProps) {
  const { id } = await params;
  const requestedParams = (await searchParams) ?? {};
  const requestedPage = parsePageParam(requestedParams.page);
  const album = await getArchivoAlbumPage(id, requestedPage);

  if (!album) {
    notFound();
  }

  if (album.post.kind !== ARCHIVO_ALBUM_KIND) {
  ...
  if (requestedPage > album.totalPages) {
    redirect(`${ARCHIVO_PATH}/album/${album.post.id}?page=${album.totalPages}`);
  }
    ...
    <main className="min-h-screen min-h-dvh overflow-x-hidden bg-black text-white">
      <div className="mx-auto min-h-screen min-h-dvh w-full max-w-2xl border-neutral-800 bg-black sm:border-x">
        ...
        <article className="border-b border-neutral-800 px-4 py-5">
          ...
          <NumberedPagination
            basePath={`${ARCHIVO_PATH}/album/${post.id}`}
            page={page}
